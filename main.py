...
class Translocator(BaseModel):
    ...
    def get_2d_dist(self) -> int:
        return int(
            math.sqrt(
                (self.destination.x - self.origin.x) ** 2
                + (self.destination.z - self.origin.z) ** 2
            )
        )
...
class Landmark(BaseModel):
    ...
    def location(self) -> Point:
        return Point.from_string(self.loc_str)
...
for tl in tls:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            marker=dict(
                size=12, color="white", symbol="circle-open", line=dict(width=2)
            ),
            hoverlabel=dark_hover,
            hoverinfo="text",
            text=[
                f"{tl.name} (Origin)<br>{tl.origin.tooltip()}",
                f"{tl.name} (Dest)<br>{tl.destination.tooltip()}",
            ],
            showlegend=False,
            legendgroup=tl.name,
        )
